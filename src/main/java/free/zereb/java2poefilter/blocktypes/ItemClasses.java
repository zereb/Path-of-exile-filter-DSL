package free.zereb.java2poefilter.blocktypes;

import java.io.IOException;
import java.net.URI;
import java.net.http.HttpClient;
import java.net.http.HttpRequest;
import java.net.http.HttpResponse;

public class ItemClasses {
    public static final String url = "https://pastebin.com/raw/KnZqvken";


    public static final String AbyssJewel ="Abyss Jewel";
    public static final String ActiveSkillGems ="Active Skill Gems";
    public static final String Amulets ="Amulets";
    public static final String Axes ="Axes";
    public static final String Belts ="Belts";
    public static final String BodyArmours ="Body Armours";
    public static final String Boots ="Boots";
    public static final String Bows ="Bows";
    public static final String Claws ="Claws";
    public static final String Currency ="Currency";
    public static final String Daggers ="Daggers";
    public static final String DelveSocketableCurrency ="Delve Socketable Currency";
    public static final String DelveStackableSocketableCurrency ="Delve Stackable Socketable Currency";
    public static final String DivinationCard ="Divination Card";
    public static final String FishingRods ="Fishing Rods";
    public static final String Flasks ="Flasks";
    public static final String Gems ="Gems";
    public static final String Gloves ="Gloves";
    public static final String Helmets ="Helmets";
    public static final String HybridFlasks ="Hybrid Flasks";
    public static final String Incubator ="Incubator";
    public static final String IncursionItem ="Incursion Item";
    public static final String Jewel ="Jewel";
    public static final String LabyrinthItem ="Labyrinth Item";
    public static final String LabyrinthMapItem ="Labyrinth Map Item";
    public static final String LabyrinthTrinket ="Labyrinth Trinket";
    public static final String LargeRelics ="Large Relics";
    public static final String Leaguestones ="Leaguestones";
    public static final String LifeFlasks ="Life Flasks";
    public static final String Maces ="Maces";
    public static final String ManaFlasks ="Mana Flasks";
    public static final String MapFragments ="Map Fragments";
    public static final String Maps ="Maps";
    public static final String MediumRelics ="Medium Relics";
    public static final String MiscMapItems ="Misc Map Items";
    public static final String OneHandAxes ="One Hand Axes";
    public static final String OneHandMaces ="One Hand Maces";
    public static final String OneHandSwords ="One Hand Swords";
    public static final String PantheonSoul ="Pantheon Soul";
    public static final String Piece ="Piece";
    public static final String QuestItems ="Quest Items";
    public static final String Quivers ="Quivers";
    public static final String Relics ="Relics";
    public static final String Rings ="Rings";
    public static final String Sceptres ="Sceptres";
    public static final String Shields ="Shields";
    public static final String SmallRelics ="Small Relics";
    public static final String StackableCurrency ="Stackable Currency";
    public static final String Staves ="Staves";
    public static final String SupportSkillGems ="Support Skill Gems";
    public static final String Swords ="Swords";
    public static final String ThrustingOneHandSwords ="Thrusting One Hand Swords";
    public static final String TwoHandAxes ="Two Hand Axes";
    public static final String TwoHandMaces ="Two Hand Maces";
    public static final String TwoHandSwords ="Two Hand Swords";
    public static final String UtilityFlasks ="Utility Flasks";
    public static final String Wands ="Wands";
    public static final String Warstaves ="Warstaves";
    public static final String RuneDaggers ="Rune Daggers";
    public static final String AtlasRegionUpgradeItem ="Atlas Region Upgrade Item";
    public static final String HarvestInfrastructure  ="HarvestInfrastructure";


    //heist

    public static final String Trinket  = "Trinket";
    public static final String HeistTarget = "Heist Target";
    public static final String Contract = "Contract";
    public static final String Blueprint = "Blueprint";
    public static final String HeistCloak = "Heist Cloak";
    public static final String HeistBrooch = "Heist Brooch";
    public static final String HeistTool = "Heist Tool";
    public static final String HeistGear = "Heist Gear";

    //ritual

    //ultimatum
}
